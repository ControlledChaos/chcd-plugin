...
class Admin_Pages {
	...
	public function __construct() {
		...
		if ( chcd_plugin()->has_acf() ) {
			...
			$admin_header = get_option( 'chcd_use_admin_header' );
		}
	...
	public function about_plugin() {
		...
		// If ACF is not active, get the field from the WordPress/ClassicPress options page.
			...
			// Get the field.
			$link_label = sanitize_text_field( get_option( 'chcd_site_plugin_link_label' ) );
		}
		...
		// Otherwise use Website Help as the label.
		}  else {
			$label = __( 'Website Help', chcd_plugin() :: DOMAIN );
		}
	...
	public function admin_header_layout() {
		...
		$style .= '.gutenberg-editor-page .chcd-admin-header { padding-left: 20px; }';
		$style .= '.gutenberg-editor-page .chcd-admin-header:before { content: ""; display: block; width: 100%; height: 56px; }';

		// End the style block.
		$style .= '</style>';

		// Apply a filter for custom admin themeing.
		$style = apply_filters( 'chcd_admin_header_layout', $style );
	...
	public function title_placeholders( $title ) {
		...
		$title = apply_filters( 'chcd_post_title_placeholders', $post_title );
	...
	private function get_column_image( $post_ID ) {
		...
		$thumb = apply_filters( 'chcd_column_thumbnail_size', $size );
