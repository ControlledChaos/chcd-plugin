...
if ( isset( $chcd_order_options['objects'] ) ) {
    $chcd_order_post_types = $chcd_order_options['objects'];

// Return empty array if no registered public tpost types.
} else {
    $chcd_order_post_types = [];
}

// Set variable for array of registered public taxonomies.
if ( isset( $chcd_order_options['tags'] ) ) {
    $chcd_order_taxonomies = $chcd_order_options['tags'];
...
<div class="wrap">
    ...
    <?php endif; ?>
    <form method="post">
        <?php if ( function_exists( 'wp_nonce_field' ) ) { wp_nonce_field( 'chcd_posts_order_nonce' ); } ?>
                        ...
                        <td>
                            ...
                            foreach ( $post_types as $post_type ) :
                                ...
                                <label><input type="checkbox" name="objects[]" value="<?php echo $post_type->name; ?>" <?php
                                    if ( isset( $chcd_order_post_types ) && is_array( $chcd_order_post_types ) ) {
                                        if ( in_array( $post_type->name, $chcd_order_post_types ) ) {
                        ...
                        <td>
                            ...
                            foreach ( $taxonomies as $taxonomy ) :
                                ...
                                <label><input type="checkbox" name="tags[]" value="<?php echo $taxonomy->name; ?>" <?php
                                    if ( isset( $chcd_order_taxonomies ) && is_array( $chcd_order_taxonomies ) ) {
                                        if ( in_array( $taxonomy->name, $chcd_order_taxonomies ) ) {
                                            echo 'checked="checked"';
                                        }
...
( function ($) {
    ...
    $( '#chcd_order_check_all_post_types' ).on( 'click', function () {
    ...
    $( '#chcd_order_check_all_taxonomies' ).on( 'click', function () {
