...
// If Advanced Custom Fields is not active.
    ...
    $sort_order = get_option( 'chcd_use_custom_sort_order' );
}
...
class Post_Types_Taxes_Order {
	...
	public static function instance() {
		...
		if ( is_null( $instance ) ) {
            ...
            if ( ! get_option( 'chcd_order_install ' ) ) {
    ...
    public function order_install() {
        ...
        update_option( 'chcd_order_install', 1 );
    ...
    public function update_options() {
        ...
        if ( ! isset( $_POST['chcd_posts_order_submit'] ) ) {
            return false;
        }

        check_admin_referer( 'chcd_posts_order_nonce' );

        $input_options            = [];
        $input_options['objects'] = isset( $_POST['objects'] ) ? $_POST['objects'] : '';
        $input_options['tags']    = isset( $_POST['tags'] ) ? $_POST['tags'] : '';

        update_option( 'chcd_order_options', $input_options );
    ...
    public function get_order_options_objects() {
        ...
        if ( $chcd_order_options = get_option( 'chcd_order_options' ) ) {
            $chcd_order_options = get_option( 'chcd_order_options' );
        } else {
            $chcd_order_options = [];
        }

        if ( isset( $chcd_order_options['objects'] ) && is_array( $chcd_order_options['objects'] ) ) {
            $objects = $chcd_order_options['objects'];
        } else {
            $objects = [];
        }
    ...
    public function get_order_options_tags() {
        ...
        if ( isset( $chcd_order_options['tags'] ) && is_array( $chcd_order_options['tags'] ) ) {
            $tags = $chcd_order_options['tags'];
        } else {
            $tags = [];
        }
 ...
 */
function chcd_post_types_taxes_order() {
...
function uninstall_db() {
    ...
    delete_option( 'chcd_order_install' );
