...
namespace CHCD_Plugin\Includes\Users;
...
class User_Avatars {
	...
	public function avatar_settings_field( $args ) {
		...
		$option = get_option( 'chcd_user_avatars_caps' );

		$html = '<p><input type="checkbox" id="chcd_user_avatars_caps" name="chcd_user_avatars_caps" value="1" ' . checked( 1, $option, false ) . '/>';

		$html .= '<label for="chcd_user_avatars_caps"> ' . $args[0] . '</label></p>';
	...
	public function get_avatar( $avatar = '', $id_or_email, $size = 96, $default = '', $alt = false ) {
		...
		$local_avatars = get_user_meta( $user_id, 'chcd_user_avatar', true );
		...
		if ( empty( $local_avatars[$size] ) ) {
			...
			update_user_meta( $user_id, 'chcd_user_avatar', $local_avatars );
		...
		return apply_filters( 'chcd_user_avatar', $avatar );
	...
	public function edit_user_profile( $profileuser ) {
		...
		<table class="form-table">
			<tr>
				...
				<td>
				<?php
				$options = get_option( 'chcd_user_avatars_caps' );
				if ( empty( $options['chcd_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
					// Nonce security ftw.
					wp_nonce_field( 'chcd_user_avatar_nonce', '_chcd_user_avatar_nonce', false );
	...
	public function edit_user_profile_update( $user_id ) {
		...
		if ( ! isset( $_POST['_chcd_user_avatar_nonce'] ) || ! wp_verify_nonce( $_POST['_chcd_user_avatar_nonce'], 'chcd_user_avatar_nonce' ) )
			return;
		...
		if ( ! empty( $_FILES['basic-user-avatar']['name'] ) ) {
			...
			update_user_meta( $user_id, 'chcd_user_avatar', [ 'full' => $avatar['url'] ] );
	...
	public function avatar_delete( $user_id ) {
		...
		$old_avatars = get_user_meta( $user_id, 'chcd_user_avatar', true );
		$upload_path = wp_upload_dir();
		...
		delete_user_meta( $user_id, 'chcd_user_avatar' );
